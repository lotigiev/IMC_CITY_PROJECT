...
class Trade(DictLikeFrozenDataclassMapping):
    ...
    def __post_init__(self, **kwargs):
        """Handle any extra fields passed by the API."""
        pass
...
class _SSEThread(Thread):
    """Background thread that consumes the CMI SSE stream with robust reconnection logic."""
    ...
    def __init__(
        ...
        self._max_silence_seconds = 15.0  # Proactively reconnect if no data for 15s
    ...
    def run(self):
        ...
        while not self._closed:
            try:
                self._consume()
                # If consume returns normally, reset retry delay
                retry_delay = 1.0
            except Exception as e:
                if self._closed:
                    break
                
                print(f"SSE Connection lost ({type(e).__name__}: {e}). Reconnecting in {retry_delay}s...")
    ...
    def _consume(self):
        ...
        # Use httpx for better timeout and stream management
        # We set read timeout to None and handle silence ourselves via a watchdog pattern
        with httpx.Client(timeout=httpx.Timeout(5.0, read=20.0)) as client:
            try:
                with httpx_sse.connect_sse(client, "GET", self._url, headers=headers) as event_source:
                    self._last_event_time = time.time()
                    ...
                    for event in event_source.iter_sse():
                        ...
                        elif event.event == "ping" or not event.data:
                            # Heartbeat received
                            continue
            except (httpx.ReadTimeout, httpx.RemoteProtocolError):
                # These are expected during long-lived streams, run() will retry
                pass
